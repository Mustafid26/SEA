...
class AdminController extends Controller
{
    ...
    public function search_pretest_user(Request $request)
    {
        $search_text = $request->search_pretest_user;
        $PretestUser = PretestUser::where('name', 'LIKE', "%$search_text%")->GET();
        return view('admin.pretest_user', compact('PretestUser'));
    }
    ...
    public function search_postest_user(Request $request)
    {
        $search_text = $request->search_postest_user;
        $PostestUser = PostestUser::where('name', 'LIKE', "%$search_text%")->GET();
        return view('admin.postest_user', compact('PostestUser'));
    }
    ...
    public function add_pelatihan(Request $request)
    {
        ...
        $pelatihan->title = $request->title;
        $pelatihan->slug = $request->slug;
        $pelatihan->body = $request->body;
        ...
        $image = $request->image;
        if ($request->file('image')) {
            $imagePath = $validated['image'] = $request->file('image')->store('pp_pelatihan', 'public');
            $pelatihan->image = $imagePath;
        }

        $pelatihan->save();
        Alert::success('Success', 'Pelatihan added successfully');
        return redirect()->back();
    }
    public function show_pelatihan()
    {
        if (auth::id()) {
            $pelatihan = Pelatihan::all();
            return view('admin.show_pelatihan', compact('pelatihan'));
        } else {
            return redirect('login');
        }
    }
    public function update_pelatihan($id)
    {
        ...
        return view('admin.update_pelatihan', compact('pelatihan'));
    ...
    public function delete_pelatihan($id)
    {
        $pelatihan = Pelatihan::find($id);
        $pelatihan->delete();
        Alert::success('Success', 'Pelatihan deleted successfully');
        return redirect()->back();
    }

    public function update_pelatihan_2(Request $request, $id)
    {
        $pelatihan = Pelatihan::find($id);
        $pelatihan->title = $request->title;
        $pelatihan->slug = $request->slug;
        ...
        if ($request->file('image')) {
            // Delete the old image file from storage
            if ($pelatihan->image) {
                Storage::disk('public')->delete($pelatihan->image);
            }
            ...
            // Store the new image file
            $imagePath = $validated['image'] = $request->file('image')->storeAs('pp_pelatihan', $originalName, 'public');
    ...
    public function search_pelatihan(Request $request)
    {
        $search_text = $request->search_pelatihan;
        $pelatihan = Pelatihan::where('title', 'LIKE', "%$search_text%")->GET();
        return view('admin.show_pelatihan', compact('pelatihan'));
    }
    ...
    public function add_kelas(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $imagePath = $request->file('image')->store('photo_kelas', 'public');
