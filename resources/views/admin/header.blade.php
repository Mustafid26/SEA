<nav class="navbar p-0 fixed-top d-flex flex-row">
    <div class="navbar-brand-wrapper d-flex d-lg-none align-items-center justify-content-center">
        <a class="navbar-brand brand-logo-mini" href="/redirect">
            <img src="{{ asset('admin/assets/images/logoserat.png') }}" alt="logo"
    ...
    <div class="navbar-menu-wrapper flex-grow d-flex align-items-stretch">
        ...
        <ul class="navbar-nav navbar-nav-right ml-auto w-100">
            ...
            @elseif(Request::is('pretest_user'))
                <li class="nav-item w-100">
                    <form class="nav-link mt-2 mt-md-0 d-none d-lg-flex search" action="{{ 'search_pretest_user' }}"
                        method="GET">
                        @csrf
                        <input style="color:white;" type="text" name="search_pretest_user" class="form-control"
                            placeholder="Search Pretest User">
                        <input type="submit" value="Search" class="btn btn-danger">
                    </form>
                </li>
            @elseif(Request::is('postest_user'))
                <li class="nav-item w-100">
                    <form class="nav-link mt-2 mt-md-0 d-none d-lg-flex search" action="{{ 'search_postest_user' }}"
                        method="GET">
                        @csrf
                        <input style="color:white;" type="text" name="search_postest_user" class="form-control"
                            placeholder="Search Postest User">
                        <input type="submit" value="Search" class="btn btn-danger">
                    </form>
                </li>
            @elseif(Request::is('show_pelatihan'))
                <li class="nav-item w-100">
                    <form class="nav-link mt-2 mt-md-0 d-none d-lg-flex search" action="{{ 'search_pelatihan' }}"
                        method="GET">
                        @csrf
